 ...
 private:
  ...
  EIGEN_STRONG_INLINE byte_t* move_to_next_block(size_t bytes, std::size_t alignment) {
    ...
    if (EIGEN_PREDICT_TRUE(block_idx_ < blocks_.size())) {
      uint8_t offset = static_cast<uint8_t>(alignment - (reinterpret_cast<std::size_t>(blocks_[block_idx_]) & (alignment - 1)));
...
class polymorphic_allocator : public std::pmr::polymorphic_allocator<byte_t> {
 public:
  ...
  polymorphic_allocator() noexcept :
    std::pmr::polymorphic_allocator<byte_t>(get_default_resource()) {};
   ...
   */
  polymorphic_allocator(Eigen::memory_resource* resource) noexcept : std::pmr::polymorphic_allocator<byte_t>(resource) {}
